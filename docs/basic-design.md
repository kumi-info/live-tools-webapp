...
### 5.4 マイクラ WIN の自動反映
- 1秒ごとに CommonWIN プラグインの `<url>/wins`（`{wins,goal,seq,up}`）をポーリングし、差分を `addWins` して配信する（`via:"add"`）。
- 基準合わせ（差分を送らない）は「webapp起動直後の初回」と「プラグイン再起動を検知したとき」。
- URL側の累計WINを0にそろえる条件は2つ：①プラグイン再起動（`up` 変化。前回値は `profiles.json` に永続するため webapp 同時再起動でも検知できる）②プラグイン側が `>0 → 0`＝マイクラ側 `/win reset`。どちらも `via:"set"`（±WIN演出は鳴らさない）。
- 目標WIN数（`goal`）もプラグイン側の変更に即時追従する（`goal=0`＝未設定のときは webapp 側の設定を優先）。

### 5.5 WINカウンターのリセット単位
- **アプリ（`start.bat`）の再起動** … 起動時に `profiles.resetAllWins()` で全プロファイルのWINを0にする。
- **OBS/ブラウザのリロード** … socket再接続のみでサーバーは動き続けるため **0にならない**（表示が消えない）。
...
## 7. ベータ機能について
`ゲーム連携β`・`ガチャβ`・`メンバーレベルβ` は開発中のベータ機能のため、**公開配布版（本リポジトリ）には同梱していない**。`server.js`・`lib/tiktok.js` は該当モジュールが存在しなくても起動できるよう任意読込＋ガードで実装しており、`GET /api/features` が同梱状況（`{gamebeta,gachabeta,levelbeta}`）を返す。管理画面はこの値で☰メニュー内のβ項目の表示可否を切り替える。
