...
function createBridge(deps) {
  ...
  function connectLoop(myGen, username) {
    ...
    c.on(WebcastEvent.STREAM_END || "streamEnd", () => retry("配信終了"));
    ...
    c.connect()
      .then((state) => {
        ...
        if (!title) setTimeout(() => { try { if (myGen === gen) analytics.maybeSetTitle(username, pickRoomTitle(c, null)); } catch (_) {} }, 4000);
