...
/**
 * すべてのプロファイルの累計WINを0にする（サーバー起動時に呼ぶ想定）。
 *
 * <p>webapp（このアプリ）を再起動するたびにWINを0から始めるための処理。
 * OBS/ブラウザのリロード（サーバーは起動したまま＝socket再接続）では呼ばれないので、
 * 「リロードでは消えない／アプリ再起動では0」を両立できる。
 *
 * @return {string[]} 0にリセットしたプロファイルIDの一覧
 */
function resetAllWins() {
  const ids = [];
  for (const id of Object.keys(store.profiles)) {
    const p = store.profiles[id];
    if (p && (p.wins | 0) !== 0) { p.wins = 0; ids.push(id); }
    else if (p) { p.wins = 0; }
  }
  saveSoon();
  return ids;
...
// ===== マイクラ連携：前回同期時に見たプラグイン起動エポック（up）を永続化 =====
// これを保存しておくと、webapp を同時に再起動した場合でも「マイクラ側サーバーが再起動したか」を
// 判定でき、サーバー再起動時に URL 側の累計WINを0へリセットできる。
/** 記録済みのプラグイン起動エポックを返す。未記録なら null。 @param {string} id */
function getMcUp(id) {
  const p = store.profiles[id];
  return (p && typeof p.mcUp === "number") ? p.mcUp : null;
}
/** プラグイン起動エポックを記録する。 @param {string} id @param {number} up */
function setMcUp(id, up) {
  const p = store.profiles[id];
  if (!p) return;
  const n = Number(up);
...
module.exports = {
  ...
  getWins, setWins, addWins, resetAllWins,
  getMcUp, setMcUp,
  loadState, saveState
};
